...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<DashboardInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/dashboards` : null),
    () => getDashboards({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Dashboards',
      path: isTenantUser ? `/dashboards/view/${data?.[0]?.id}` : '/dashboards',
      entity: 'dashboard',
    ...
    { name: 'Categories', path: '/categories', entity: 'category', service: AccessServiceEnum.PROJECT, icon: FiList },
    {
      name: 'Communities',
      path: '/communities',
      entity: 'community',
    ...
    { name: 'Resources', path: '/resources', entity: 'resource', service: AccessServiceEnum.PROJECT, icon: FiBookOpen },
    { name: 'Skills', path: '/skills', entity: 'skill', service: AccessServiceEnum.PROJECT, icon: FiStar },
