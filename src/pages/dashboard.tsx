...
import { getAchievements } from 'apiSdk/achievements';
import { AchievementInterface } from 'interfaces/achievement';

import { CommunityListPage as ShowCaseList } from 'pages/communities';

function DashboardPage() {
  const { session } = useSession();

  const communityStat = useSWR<PaginatedInterface<CommunityInterface>>(
    () => `/communities`,
    () => getCommunities({ limit: 1, offset: 0 }),
  );
  ...
  const achievementStat = useSWR<PaginatedInterface<AchievementInterface>>(
    () => `/achievements`,
    () => getAchievements({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Communities',
      result: communityStat?.data?.totalCount,
      isLoading: communityStat.isLoading,
    },
    {
      title: 'Categories',
    ...
    {
      title: 'Achievements',
      result: achievementStat?.data?.totalCount,
      isLoading: achievementStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`communities`} passHref legacyBehavior>
                  <Button
