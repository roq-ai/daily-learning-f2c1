...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'category.create': {
    ...
    key: 'new-category-added',
    tenantPath: ['dashboard', 'user', 'category'],
    userPath: [],
  },
  'skill.update': {
    roles: ['team-member'],
    key: 'skill-updated',
    tenantPath: ['dashboard', 'user', 'skill'],
    userPath: [],
  },
  'achievement.create': {
    roles: ['team-member'],
    key: 'new-achievement-unlocked',
    tenantPath: ['dashboard', 'user', 'achievement'],
    userPath: [],
  },
};
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.dashboard.findFirst({
